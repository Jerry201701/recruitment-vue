	...
	<el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="editDialogVisible" :close-on-click-modal="false">
		<el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size">
			...
			<el-form-item label="名称" prop="regionName">
				<el-input v-model="dataForm.regionName" auto-complete="off"></el-input>
			</el-form-item>
...
export default {
	...
	data() {
		return {
			...
			columns: [
				...
				{prop:"regionName", label:"区域名称", minWidth:100},
	...
	methods: {
		...
		findPage: function (data) {
			...
			this.$api.job.pageRecruitmentRegion(this.pageRequest).then((res) => {
				this.pageResult = res.data
			}).then(data!=null?data.callback:'')
		},
		// 批量删除
		handleDelete: function (data) {
			this.$api.job.deleteRecruitmentRegion(data.params).then(data!=null?data.callback:'')
		},
		// 显示新增界面
		handleAdd: function () {
			...
			this.$refs.dataForm.validate((valid) => {
				if (valid) {
					this.$confirm('确认提交吗？', '提示', {}).then(() => {
						...
						this.$api.job.addRecruitmentRegion(params).then((res) => {
							if(res.code == 200) {
